...
export class MemoryMessagingDriver implements MessagingDriver {
    ...
    private readonly logger = LoggerManager.resolveLogger({ context: 'MemoryMessagingDriver' });
    ...
    async publish(envelope: MessageEnvelope, options?: PublishOptions): Promise<void> {
        if (!options?.exchange) {
            throw new Error('Exchange must be specified in PublishOptions for Memory driver');
        }
    ...
    async subscribe(
        ...
        const retryPolicy: RetryPolicy = {
            maxRetries: options?.retryPolicy?.maxRetries ?? this.defaultRetryPolicy.maxRetries,
            delayTiers: options?.retryPolicy?.delayTiers ?? this.defaultRetryPolicy.delayTiers,
        };

        if (!options?.exchange) {
            throw new Error('Exchange must be specified in SubscribeOptions for Memory driver');
        }
